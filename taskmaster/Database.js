function dbInit()
{
    var db = LocalStorage.openDatabaseSync("Task_Tracker_DB", "", "Track Task", 1000000)
    try {
        db.transaction(function (tx) {
            tx.executeSql('CREATE TABLE IF NOT EXISTS trip_log (date text,trip_desc text, distance text)')
        })
    } catch (err) {
...
{
    try {
        var db = LocalStorage.openDatabaseSync("Task_Tracker_DB", "",
                                               "Track Task", 1000000)
    } catch (err) {
...
function dbDeleteRowAll()
{
    var db = dbGetHandle()
    db.transaction(function (tx) {
        tx.executeSql('delete from trip_log')
    })
}
